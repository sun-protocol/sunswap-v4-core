...
library BinPool {
    ...
    {   
        ...
        //确认方向，如果 amountSpecified 是负数，则这是一次“精确输入”（Exact Input）交易。
        bool exactInput = params.amountSpecified < 0;
        
        {
            ////根据beforeswap的动作更新lpfee
        ...
        // amountsLeft = |-- 128bit encodeFrist --| -- 128bit encodeFrist --|
        ...
        // amountsUnspecified = |-- 128bit encodeSecond --| -- 128bit encodeFrist --|
        bytes32 amountsUnspecified;
        //进入循环
        while (true) {
            //获取当前activeId box 的仓位
            bytes32 binReserves = self.reserveOfBin[swapState.activeId];
            //为啥用双重否定判断？
            // 如果 反向 binReserves 不为空，则继续处理
            if (!binReserves.isEmpty(!swapForY)) {
                bytes32 amountsInWithFees;  // in
                bytes32 amountsOutOfBin;    // out
                bytes32 totalFee;           // fee
                //如果是exactInput
                if (exactInput) {
