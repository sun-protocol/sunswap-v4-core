...
contract Fuzzers is StdUtils {
    ...
    function boundLiquidityDelta(PoolKey memory key, int256 liquidityDeltaUnbounded, int256 liquidityMaxByAmount)
        ...
        returns (int256)
    {
        int256 liquidityMaxPerTick =
            int256(uint256(Tick.tickSpacingToMaxLiquidityPerTick(key.parameters.getTickSpacing())));
    ...
    function boundLiquidityDeltaTightly(
        ...
        int256 liquidityMaxTightBound =
            int256(uint256(Tick.tickSpacingToMaxLiquidityPerTick(key.parameters.getTickSpacing())) / maxPositions);
    ...
    function boundTicks(PoolKey memory key, int24 tickLower, int24 tickUpper) internal pure returns (int24, int24) {
        return boundTicks(tickLower, tickUpper, key.parameters.getTickSpacing());
    }
    ...
    function createRandomSqrtPriceX96(int24 tickSpacing, int256 seed) internal pure returns (uint160) {
        ...
        return TickMath.getSqrtRatioAtTick(int24(randomTick));
    }
    ...
    function createFuzzyLiquidity(
        CLPoolManagerRouter modifyLiquidityRouter,
        PoolKey memory key,
        ICLPoolManager.ModifyLiquidityParams memory params,
        uint160 sqrtPriceX96,
        bytes memory hookData
    ) internal returns (ICLPoolManager.ModifyLiquidityParams memory result, BalanceDelta delta) {
        result = createFuzzyLiquidityParams(key, params, sqrtPriceX96);
        (delta,) = modifyLiquidityRouter.modifyPosition(key, result, hookData);
    }
