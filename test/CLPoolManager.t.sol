...
import "src/interfaces/ICLHooks.sol";
...
contract CLPoolManagerTest is Test, NoIsolate, Deployers, TokenFixture {
    ...
    using CLPoolGetter for PoolManager;
    ...
    function test_initialize_fuzz(PoolKey memory key, uint160 sqrtPriceX96) public {
        ...
        if (key.parameters.getTickSpacing() > TickMath.MAX_TICK_SPACING) {
            ...
            emit ICLPoolManager.Initialize(
                key.toId(), key.currency0, key.currency1, key.hooks, key.fee, key.parameters, sqrtPriceX96, tick, 1
            );
            poolManager.initialize(key, sqrtPriceX96);
    ...
    function test_initialize_succeedsWithHooks(uint160 sqrtPriceX96) public {
        ...
        (Currency curr0, Currency curr1, IHooks hooks, uint24 fee, bytes32 parameters) =
    ...
    function testModifyPosition_removeLiquidity_empty() external {
        ...
        assertEq(IERC20(Currency.unwrap(currency0)).balanceOf(address(poolManager)), 4999625031247266);
        assertEq(IERC20(Currency.unwrap(currency1)).balanceOf(address(poolManager)), 4999625031247266);
        ...
        assertEq(IERC20(Currency.unwrap(currency0)).balanceOf(address(poolManager)), 1);
        assertEq(IERC20(Currency.unwrap(currency1)).balanceOf(address(poolManager)), 1);
  ...
  // unknow  balanceDeltas ，as default false
    ...
    function testSwap_runOutOfLiquidity() external {
        ...
        assertEq(IERC20(Currency.unwrap(currency0)).balanceOf(address(poolManager)), 4977594234867895338);
        // token1: roughly 502 ether
        assertEq(IERC20(Currency.unwrap(currency1)).balanceOf(address(poolManager)), 502165582277283491084);
    ...
    function testSwap_leaveSurplusTokenInpoolManager() public {
        PoolKey memory key = PoolKey({
            currency0: currency0,
            currency1: currency1,
            fee: 3000,
        ...
        uint256 surplusTokenAmount = poolManager.balanceOf(address(this), currency1);
    ...
    function testSwap_useSurplusTokenAsInput() public {
        ...
        poolManager.approve(address(router), currency0, type(uint256).max);
        poolManager.approve(address(router), currency1, type(uint256).max);
        ...
        surplusTokenAmount = poolManager.balanceOf(address(this), currency1);
    ...
    function testSwap_leaveSurplusTokenInpoolManager_gas() public {
        PoolKey memory key = PoolKey({
            currency0: currency0,
            currency1: currency1,
            fee: 3000,
        ...
        router.swap(key, params, testSettings, ZERO_BYTES);
        vm.snapshotGasLastCall("swap_leaveSurplusTokenInpoolManager");
    }
