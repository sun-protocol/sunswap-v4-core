...
contract CLMintBurnFeeHookTest is Test, Deployers, TokenFixture {
    ...
    function setUp() public {
        ...
        clMintBurnFeeHook = new CLMintBurnFeeHook(poolManager, poolManager);
    ...
    function test_Mint() external {
        ...
        assertEq(token0.balanceOf(address(poolManager)), 1499550104979003780); // ~1.5 ether
        assertEq(token1.balanceOf(address(poolManager)), 1499550104979003780); // ~1.5 ether

        // hook mint VaultToken instead of taking token from vault as vault does not have token in this case
        assertEq(poolManager.balanceOf(address(clMintBurnFeeHook), key.currency0), 999700069986002520); // ~1 eth
        assertEq(poolManager.balanceOf(address(clMintBurnFeeHook), key.currency1), 999700069986002520); // ~1 eth
    }
    ...
    function test_Burn() external {
        ...
        assertEq(token0.balanceOf(address(poolManager)), 2999100209958007557); // ~3 eth
        assertEq(token1.balanceOf(address(poolManager)), 2999100209958007557); // ~3 eth

        // 1 to 3 eth = 2 eth diff :: + 2 eth as fee from remove liquidity
        assertEq(poolManager.balanceOf(address(clMintBurnFeeHook), key.currency0), 2999100209958007556); // ~3 eth
        assertEq(poolManager.balanceOf(address(clMintBurnFeeHook), key.currency1), 2999100209958007556); // ~3 eth
    }
